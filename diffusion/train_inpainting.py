...
from lib.train_diffusion_inpainting import TrainDiffDataset
from lib.test_diffusion_inpainting import TestDiffDataset
...
def main(args, args_str):
    ...
    # ... (keep the checkpoint loading code the same)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                latents = vae.encode(batch["tgt_img"].to(dtype=weight_dtype)).latent_dist.sample()
                ...
                # Assuming inpaint_mask is [batch_size, 1, height, width] and needs to be downsampled to [batch_size, 1, 64, 64]
                inpaint_mask= F.interpolate(batch['inpaint_mask'], size=(64, 64), mode='bilinear', align_corners=False)


                print("Model Prediction Shape:", model_pred.shape)
                print("Target Shape:", target.shape)
                print("Inpaint Mask Shape:", batch['inpaint_mask'].shape)

                # # Calculate loss using inpaint_mask
                loss = F.mse_loss(model_pred.float() * inpaint_mask, target.float() * inpaint_mask, reduction="mean")
